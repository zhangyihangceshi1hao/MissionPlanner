                ...
                //Thread.Sleep(1000); // 模拟工作1秒钟
                    ...
                    foreach (var port in MainV2.Comports)
                    {
                        ...
                        PdxpPacket2 pdxpPacket2 = new PdxpPacket2
                        {
                            VER = 0x80,                    // 协议版本号
                            MID = 10576,               // 任务代号
                            SID = BitConverter.ToInt32(new byte[4] { 0xee, 0x11, 0x02, 0x01 }, 0),           // 发送方地址
                            DID = BitConverter.ToInt32(new byte[4] { 0xee, 0x11, 0x02, 0x01 }, 0),           // 接收方地址
                            BID = BitConverter.ToInt32(new byte[4] { 0xee, 0x2b, 0x02, mav.sysid }, 0),           // 数据包类型标识
                            No = (Int32)sequence,                  // 包序号
                            FLAG = 0,                // 标志位
                            R = 0,                      // 预留或使用字段
                            DATE = (ushort)(DateTime.Now - new DateTime(2000, 1, 1)).Days, // 从2000年累计天数
                            ...
                            AirplaneMode = 0x01,        // 飞行模式
                            Year = (byte)nowUtc.Year,                 // UTC年份（如 2024）

...
using System.Linq;
using static MissionPlanner.Swarm.FormationControls;
...
namespace MissionPlanner.Swarm
{
    ...
    public class Formation : Swarm
    {
        ...
        public  void UpdateSwarms(MAVState mavSwarms)
        {
            MAVState targetMav = MainV2.Comports
                .SelectMany(port => port.MAVlist)  // 合并所有 MAVlist 列表
                .FirstOrDefault(mav => mav == mavSwarms);  // 查找 sysid 匹配的 MAVState
            if (targetMav.cs.lat == 0 || targetMav.cs.lng == 0)
                return;
        ...
        public  void SendCommandSwarms(List<MAVState> mavSwarmsList)
        {
            if (masterpos.Lat == 0 || masterpos.Lng == 0)
                return;
